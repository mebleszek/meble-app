...
async function main(){
  ...
  assert(/^wycena_diag_20260616_czynnosci_project_preparation_v1_\d{8}_\d{6}\.txt$/.test(filename), 'Nazwa pliku raportu ma zawierać build i timestamp', filename);

  const index = fs.readFileSync(path.join(process.cwd(), 'index.html'), 'utf8');
  const devTests = fs.readFileSync(path.join(process.cwd(), 'dev_tests.html'), 'utf8');
  assert(index.includes('js/app/wycena/wycena-diagnostics.js?v=20260616_czynnosci_project_preparation_v1'), 'index.html nie ładuje diagnostyki z cache-bustingiem');
  assert(devTests.includes('js/app/wycena/wycena-diagnostics.js?v=20260616_czynnosci_project_preparation_v1'), 'dev_tests.html nie ładuje diagnostyki z cache-bustingiem');
