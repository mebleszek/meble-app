  ...
  return makeSingleGroupReport('MATERIAŁ node smoke testy', 'Materiał ↔ Node smoke', [
    ...
    { name:'Słowniki okuć i Wzorce UI trzymają akordeon ROZRYS z ruchem', explain:'Chroni UX na telefonie: wspólny akordeon kategorii ma chevron jak ROZRYS, wzorzec UI pokazuje płynne otwarcie, a parametry nie tworzą małego wewnętrznego scrolla.', check:()=> {
      ...
      return dictionariesSrc.includes('panel-box-form__scroll hardware-dictionary-scroll')
        ...
        && !css.includes('.hardware-dictionary-categories-card > .hardware-dictionary-categories-body{display:grid;gap:10px;border-top:1px solid rgba(15,23,42,.10);padding:10px 20px 20px;background:#fff;border-bottom-left-radius:20px;border-bottom-right-radius:20px;height:auto!important')
        && css.includes('max-height:none')
        && css.includes('height:auto')
        && css.includes('.hardware-dictionary-categories-card.hardware-categories-animating > .hardware-dictionary-categories-body')
        && css.includes('transition:max-height .42s cubic-bezier')
