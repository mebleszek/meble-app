...
function runProjectNodeSmoke(sandbox){
  ...
  return makeSingleGroupReport('PROJECT node smoke testy', 'Projekt ↔ Node smoke', [
    ...
    { name:'Preferencje pokoju normalizują się w projekcie', explain:'Pilnuje pola room.preferences jako części modelu projektu, bez osobnego storage — także preferencji producentów okuć.', check:()=> {
      const out = FC.projectModel.normalizeProjectData({ schemaVersion:9, kuchnia:{ cabinets:[], fronts:[], sets:[], settings:{}, preferences:{ bodyColor:'Czarny', openingSystemModule:'TIP-ON', hardwareProducers:{ hinges:'GTV', accessories:'Rejs' }, hardwareDrawerSystems:{ drawers:'blum_tandembox_antaro' } } } });
      return !!(out && out.kuchnia && out.kuchnia.preferences && out.kuchnia.preferences.zones && out.kuchnia.preferences.zones.lower.bodyColor === 'Czarny' && out.kuchnia.preferences.zones.middle.openingSystem === 'TIP-ON' && out.kuchnia.preferences.hardwareProducers && out.kuchnia.preferences.hardwareProducers.hinges === 'GTV' && out.kuchnia.preferences.hardwareProducers.accessories === 'Rejs' && out.kuchnia.preferences.hardwareDrawerSystems && out.kuchnia.preferences.hardwareDrawerSystems.drawers === 'blum_tandembox_antaro' && out.szafa && out.szafa.preferences && out.szafa.preferences.zones && out.szafa.preferences.hardwareProducers && out.szafa.preferences.hardwareDrawerSystems);
    } },
    ...
    { name:'Boot startuje init dopiero po DOMContentLoaded', explain:'Chroni mobilny świeży deploy: boot.js ładuje się jako pierwszy defer i nie może uruchamiać init ani zgłaszać braku init, zanim app.js oraz reszta deferred scripts skończą rejestrację.', check:()=> {
      const src = fs.readFileSync(path.join(process.cwd(), 'js/boot.js'), 'utf8');
      return src.includes("boot-clean-1.6")
        && src.includes("if (document.readyState === 'complete')")
        && src.includes("document.addEventListener('DOMContentLoaded', boot")
        && src.includes('if(!loadSeen) return false')
        && src.includes('Date.now() - loadAt >= BOOT_MISSING_INIT_TIMEOUT_MS + BOOT_MISSING_INIT_LOAD_GRACE_MS')
        && !src.includes("if (document.readyState === 'loading')")
...
function runMaterialNodeSmoke(sandbox){
  ...
  return makeSingleGroupReport('MATERIAŁ node smoke testy', 'Materiał ↔ Node smoke', [
    ...
    { name:'Dynamiczne parametry techniczne okuć mają słownik i helpy', explain:'Chroni nową architekturę: kategorie mają własne parametry, nazwa techniczna powstaje z parametrów oznaczonych jako budujące nazwę, a opcje techniczne mają opisy pod ?.', check:()=> {
      ...
      const typeLabel = api.buildTypeLabel(api.DEFAULT_DEFINITIONS, 'Zawiasy', { nalozenie:{ value:'nakładany' }, kat_rzeczywisty:{ from:110, to:'' }, klasa_kata:{ value:'standardowy 90–120°' }, hamulec:{ value:true }, typ_prowadnika:{ value:'standardowy' }, forma_prowadnika:{ value:'krzyżowy' } });
      return hingeFields.some((row)=> row.key === 'kat_rzeczywisty' && row.compareMode === 'ignore')
        ...
        && hingeFields.some((row)=> row.key === 'typ_prowadnika' && row.compareMode === 'equal' && row.keyFeature === true)
        && hingeFields.some((row)=> row.key === 'forma_prowadnika' && row.compareMode === 'equal' && row.keyFeature === true)
        && hingeFields.some((row)=> row.key === 'system_kompatybilnosci' && row.compareMode === 'equal' && row.keyFeature === true)
    ...
    { name:'Cennik robocizny ma aplikacyjne launchery wyboru zamiast natywnych selectów', explain:'Chroni formularz reguł robocizny przed systemowym pickerem Android/iOS.', check:()=> {
      const src = fs.readFileSync(path.join(process.cwd(), 'js/app/material/price-modal-item-form.js'), 'utf8');
      const conditions = fs.readFileSync(path.join(process.cwd(), 'js/app/material/price-modal-labor-conditions.js'), 'utf8');
      const required = ['laborRateType','laborTimeBlockHours','laborQuantitySource','laborQuantityMode','laborStartHours','laborStepHours','laborVolumeTimeMode'];
      return src.includes('mountLaborChoiceLaunchers') && src.includes('ensureLaborChoiceMount') && src.includes('selectEl.hidden = true') && src.includes('hideLaborUsageField') && !src.includes('laborAutoRole') && required.every((id)=> src.includes(id)) && conditions.includes('select.hidden = true') && conditions.includes('Wybierz wartość warunku') && !conditions.includes('Dodaj warunek');
    } },
  ]);
}
...
function runWycenaNodeSmoke(sandbox){
  ...
  return makeSingleGroupReport('WYCENA node smoke testy', 'Wycena ↔ Node smoke', [
    ...
    { name:'Wycena core ma spójny świeży cache-busting', explain:'Chroni pierwsze odświeżenie po wdrożeniu przed mieszaniem starych i nowych modułów wycena-core*.', check:()=> {
      ...
      const currentExpected = '20260628_drawer_systems_materials_v1';
      const registerExpected = '20260628_drawer_systems_materials_v1';
      ...
      const auditMaterialsExpected = '20260628_drawer_systems_materials_v1';
      const pcvTruthExpected = '20260601_pcv_single_source_truth_v1';
      const hingeCatalogExpected = '20260628_drawer_systems_materials_v1';
      const singleTruthPreLaborExpected = '20260628_drawer_systems_materials_v1';
      const laborSingleTruthExpected = '20260628_drawer_systems_materials_v1';
  ...
  return makeSingleGroupReport('SZAFKI node smoke testy', 'Szafki ↔ Node smoke', [
    ...
    { name:'Modal szafki pokazuje panel wymagań technicznych bez produktów katalogowych', explain:'Chroni etap WYWIAD → WYCENA: użytkownik widzi cechy techniczne szafki, a nie producenta/model dobrany dopiero w WYCENIE.', check:()=> {
      ...
      return html.includes('id="cmHardwareRequirements"')
        && html.includes('cabinet-hardware-requirements-panel.js?v=20260628_drawer_systems_materials_v1')
        ...
        && container.innerHTML.includes('110°')
        && container.innerHTML.includes('Zmień')
        && container.innerHTML.includes('Przywróć domyślne')
