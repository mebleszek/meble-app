...
async function main(){
  ...
  const FC = sandbox.FC || {};
  if(!(FC.serviceDevTests && typeof FC.serviceDevTests.runAll === 'function')) throw new Error('Brak FC.serviceDevTests.runAll');
  const report = await FC.serviceDevTests.runAll();
  printReport(report);
