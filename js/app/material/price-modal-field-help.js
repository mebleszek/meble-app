...
(function(){
  ...
  const FIELD_HELP = {
    ...
    formServicePrice:{ title:'Cena / kwota', message:'Znaczenie tego pola zależy od sposobu naliczania ceny: kwota stała, cena za jednostkę albo dopłata w trybie zaawansowanym.' },
    laborPricingMode:{ title:'Sposób naliczania ceny', message:'Wybierz, jak program ma policzyć pozycję: stała kwota, cena za ilość, kwota startowa plus jednostki, albo czas liczony wybraną stawką godzinową. Formularz pokazuje tylko pola używane w wybranym sposobie.' },
    ...
    laborTimeBlockHours:{ title:'Czas bazowy', message:'Podstawowy czas doliczany dla jednej czynności albo jednego zastosowania reguły. Wybór jest pokazany w minutach; wewnętrznie program przelicza go na godziny do mnożenia przez stawkę PLN/h. Jeśli pozycja ma być czysto kwotowa, ustaw Brak.' },
    ...
    laborQuantitySource:{ title:'Źródło ilości', message:'Wybierz, z jakiej nazwanej wartości program ma brać ilość dla tej czynności, np. liczbę korpusów, frontów, zawiasów, półek albo szuflad. Warunki zastosowania decydują, kiedy taka reguła ma działać.' },
    laborQuantityMode:{ title:'Tryb ilości', message:'Techniczny tryb czasu/ilości używany głównie w trybie zaawansowanym. W zwykłych sposobach naliczania program ustawia go automatycznie.' },
    laborStartPrice:{ title:'Kwota startowa', message:'Jednorazowa kwota doliczana po wybraniu sposobu „Kwota startowa + cena za ilość”, np. minimalna opłata za transport.' },
    laborIncludedQty:{ title:'Ilość w cenie startowej', message:'Ile jednostek obejmuje kwota startowa. Przykład: 10 km w cenie startowej, a dopiero kolejne kilometry liczone są według ceny za km.' },
    laborTierText:{ title:'Progi ilościowe', message:'Tu wpisujesz pakiety ilościowe dla trybu progowego. Przykład: 1-2=15 min;3-5=30 min;6-10=60 min oznacza, ile czasu doliczyć dla danych zakresów ilości. Stare wartości w godzinach typu 0.25 nadal są odczytywane.' },
    laborStartHours:{ title:'Czas startowy', message:'Początkowy czas przy trybie start + krok. Wybór jest pokazany w minutach, a program przelicza go na godziny do stawki PLN/h.' },
    laborStartQty:{ title:'Start szt.', message:'Od ilu sztuk zaczyna działać czas startowy.' },
    laborStepEveryQty:{ title:'Co ile szt.', message:'Co ile kolejnych sztuk program ma doliczać następny krok czasu.' },
    laborStepHours:{ title:'Czas za krok', message:'Ile minut doliczać za każdy kolejny krok ilości w trybie start + krok. Program przelicza wybór na godziny do stawki PLN/h.' },
    laborVolumePricePerM3:{ title:'Dopłata zł za gabaryt', message:'Stała dopłata pieniężna liczona od objętości szafki. Nie działa równocześnie z gabarytoczasem — jeśli gabaryt liczysz jako czas, ta dopłata zostaje wyłączona, żeby nie policzyć gabarytu podwójnie.' },
    ...
    laborVolumeTimeTierText:{ title:'Progi gabarytoczasu', message:'Zakresy objętości i przypisany do nich czas. Przykład: 0.31-0.60=15 min;0.61-1=30 min. Stare wartości w godzinach typu 0.25 nadal są odczytywane.' },
