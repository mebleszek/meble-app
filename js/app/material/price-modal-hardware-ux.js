...
(function(){
  ...
  function renderHardwareAccessoryRow(item, onEdit){
    ...
    left.appendChild(renderMetaLine([text(item && item.manufacturer) || '—', text(item && item.hardwareCategory) || 'Inne', text(item && item.hardwareType) ? 'Tech: ' + text(item.hardwareType) : '', text(item && item.hardwareUnit) || 'szt.', text(item && (item.hardwareSystem || item.series)), text(item && item.symbol) ? 'SYM: ' + text(item.symbol) : '']));
