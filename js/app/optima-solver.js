...
(function(root){
  ...
  function singleItemPlacement(it, W, H){
    if(!it) return null;
    const base = [{ w: Number(it.w) || 0, h: Number(it.h) || 0, rotated:false }];
    if(it.rotationAllowed && Number(it.w) !== Number(it.h)) base.push({ w: Number(it.h) || 0, h: Number(it.w) || 0, rotated:true });
    let best = null;
    for(const c of base){
      if(c.w <= 0 || c.h <= 0) continue;
      if(c.w > W || c.h > H) continue;
      const score = (c.w * c.h) * 1000 + c.w * 10 + c.h;
      if(!best || score > best.score) best = { score, cand: c };
    }
    return best ? best.cand : null;
  }

  function buildForcedSingleSheet(it, W, H){
    const cand = singleItemPlacement(it, W, H);
    if(!cand) return null;
    return {
      boardW: W,
      boardH: H,
      ...
      _usedArea: cand.w * cand.h,
      _freeRects: [],
      _swapUsed: false,
    };
  }

  function sanitizeSheets(sheets, W, H){
    const out = [];
    for(const sheet of Array.isArray(sheets) ? sheets : []){
      if(!sheet) continue;
      const placements = Array.isArray(sheet.placements) ? sheet.placements : [];
      const visible = placements.filter(p => p && !p.unplaced);
      if(visible.length){
        sheet.placements = visible;
        if(!Number.isFinite(Number(sheet._usedArea)) || Number(sheet._usedArea) <= 0){
          sheet._usedArea = visible.reduce((sum, p)=> sum + ((Number(p.w) || 0) * (Number(p.h) || 0)), 0);
        }
        out.push(sheet);
        continue;
      }
      const unplaced = placements.filter(p => p && p.unplaced);
      for(const p of unplaced){
        const salvage = buildForcedSingleSheet({
          id: p.id,
          key: p.key,
          name: p.name,
          w: p.srcW || p.w,
          ...
          edgeH1: !!p.edgeH1,
          edgeH2: !!p.edgeH2,
        }, Number(sheet.boardW) || W, Number(sheet.boardH) || H);
        if(salvage) out.push(salvage);
      }
    }
    return out;
  }
  ...
  function packOptimaCore(itemsIn, boardW, boardH, kerf, options, skipTailPolish){
    ...
    while(remMaster.length){
      ...
      if(!attempts.length){
        ...
        if(!it) break;
        const forced = buildForcedSingleSheet(it, W, H);
        if(forced) sheets.push(forced);
        continue;
      }
    ...
    return sanitizeSheets(sheets, W, H);
  }

  function packOptima(itemsIn, boardW, boardH, kerf, options){
    return sanitizeSheets(packOptimaCore(itemsIn, boardW, boardH, kerf, options || {}, false), clampInt(boardW, 2800), clampInt(boardH, 2070));
  }
