...
(function(root){
  ...
  function singleItemPlacement(it, W, H){
    if(!it) return null;
    const options = [{ w: Number(it.w) || 0, h: Number(it.h) || 0, rotated:false }];
    if(it.rotationAllowed && Number(it.w) !== Number(it.h)) options.push({ w: Number(it.h) || 0, h: Number(it.w) || 0, rotated:true });
    let best = null;
  ...
  function sanitizeSheets(sheets, W, H){
    ...
    for(const sheet of Array.isArray(sheets) ? sheets : []){
      ...
      if(visible.length){
        ...
        if(!sheet.boardW) sheet.boardW = W;
        if(!sheet.boardH) sheet.boardH = H;
        out.push(sheet);
        continue;
      }
