(function(){
  ...
  const coreApi = (FC.roomRegistryCore && typeof FC.roomRegistryCore.createApi === 'function')
    ? FC.roomRegistryCore.createApi({ FC, getProject, saveProject, getCurrentInvestor })
    : {};
  const removalApi = (FC.roomRegistryRemoval && typeof FC.roomRegistryRemoval.createApi === 'function')
    ? FC.roomRegistryRemoval.createApi({ FC, getProject, saveProject, getCurrentInvestor, core:coreApi })
    : {};

  const BASE_LABELS = coreApi.BASE_LABELS || {
    kuchnia:'Kuchnia',
    szafa:'Szafa',
    pokoj:'Pokój',
    lazienka:'Łazienka'
  };
  const ensureProjectMeta = coreApi.ensureProjectMeta || function(proj){ return proj && proj.meta ? proj.meta : null; };
  const discoverProjectRoomKeys = coreApi.discoverProjectRoomKeys || function(){ return []; };
  const roomTemplate = coreApi.roomTemplate || function(){ return { cabinets:[], fronts:[], sets:[], settings:{} }; };
  const makeRoomId = coreApi.makeRoomId || function(baseType, name){ return 'room_' + String(baseType || 'pokoj') + '_' + String(name || 'pomieszczenie'); };
  const normalizeLabel = coreApi.normalizeLabel || function(text){ return String(text || '').trim().replace(/\s+/g, ' '); };
  const normalizeComparableLabel = coreApi.normalizeComparableLabel || function(text){ return normalizeLabel(text).toLowerCase(); };
  const normalizeRoomDef = coreApi.normalizeRoomDef || function(room){ return room || null; };
  const hasLegacyKitchen = coreApi.hasLegacyKitchen || function(){ return false; };
  const createLegacyKitchenDef = coreApi.createLegacyKitchenDef || function(){ return { id:'kuchnia', baseType:'kuchnia', name:BASE_LABELS.kuchnia, label:BASE_LABELS.kuchnia, legacy:true }; };
  const hasCurrentInvestor = coreApi.hasCurrentInvestor || function(){ return !!getCurrentInvestor(); };
  const getActiveRoomDefs = coreApi.getActiveRoomDefs || function(){ return []; };
  const getActiveRoomIds = coreApi.getActiveRoomIds || function(){ return []; };
  const getRoomLabel = coreApi.getRoomLabel || function(id){ return String(id || ''); };
  const ensureRoomData = coreApi.ensureRoomData || function(){ return null; };
  const isRoomNameTaken = coreApi.isRoomNameTaken || function(){ return false; };
  const getManageableRooms = coreApi.getManageableRooms || function(){ return []; };
  const getEditableRoom = coreApi.getEditableRoom || function(){ return null; };

  const syncQuoteDraftSelectionAfterRoomChange = removalApi.syncQuoteDraftSelectionAfterRoomChange || function(){};
  const reconcileStatusesAfterRoomSetChange = removalApi.reconcileStatusesAfterRoomSetChange || function(){};
  const listSnapshotsForRoomRemoval = removalApi.listSnapshotsForRoomRemoval || function(){ return []; };
  const countCabinetsForRoomIds = removalApi.countCabinetsForRoomIds || function(){ return 0; };
  const buildRoomRemovalWarningMessage = removalApi.buildRoomRemovalWarningMessage || function(){ return { message:'', snapshots:[], cabinetCount:0 }; };
  const askDeleteRoomWithQuotes = removalApi.askDeleteRoomWithQuotes || (async function(){ return true; });
  const updateInvestorRooms = removalApi.updateInvestorRooms || function(){};
  const removeRoomById = removalApi.removeRoomById || function(){ return null; };
  const applyManageRoomsDraft = removalApi.applyManageRoomsDraft || function(){ return []; };
