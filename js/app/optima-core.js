/* optima-core.js — wspólny rdzeń algorytmu Optimax
   3 tryby (wzdłuż / w poprzek / optima) korzystają z tego samego solvera.
   Brak limitów czasu i brak końcowego porządkowania ostatnich płyt.
*/
(function(root){
  'use strict';
  ...
  function getProfileConfig(profile){
    ...
    const map = {
      A: { seedLimit: 8, rowSeedLimit: 5 },
      B: { seedLimit: 14, rowSeedLimit: 8 },
      C: { seedLimit: 24, rowSeedLimit: 12 },
      D: { seedLimit: Number.POSITIVE_INFINITY, rowSeedLimit: Number.POSITIVE_INFINITY },
    };
  ...
  function getAllowedSwaps(mode){
    const m = normalizeMode(mode);
    if(m === 'along') return [false];
    if(m === 'across') return [true];
    return [false, true];
  }
  ...
  function buildSeedCandidates(rem, BW, BH, swap, limit){
    ...
    const finiteLimit = Number.isFinite(limit) ? Math.max(1, Math.round(limit)) : null;
    const perBucketCap = finiteLimit == null ? Number.POSITIVE_INFINITY : 4;
    for(const s of out){
      const key = String(s.it.id) + ':' + (s.cand.rotated ? 'r' : 'n');
      if(seen.has(key)) continue;
      const bucketKey = s.hBucket;
      const usedInBucket = bucketCap.get(bucketKey) || 0;
      if(usedInBucket >= perBucketCap) continue;
      seen.add(key);
      bucketCap.set(bucketKey, usedInBucket + 1);
      dedup.push(s);
      if(finiteLimit != null && dedup.length >= finiteLimit) break;
    }
    return dedup;
  }
  ...
  function reconstructRow(dp, endUsed, rem, forced){
    ...
    chosen.sort((a,b)=> (Number(b && b.cand && b.cand.w) || 0) - (Number(a && a.cand && a.cand.w) || 0));
    return chosen;
  }
  ...
  function chooseBestRow(rem, BW, availableH, K, swap, cfg, threshold, mode){
    const seedLimit = Number.isFinite(Number(cfg && cfg.rowSeedLimit)) ? Math.max(3, Math.round(Number(cfg.rowSeedLimit))) : Number.POSITIVE_INFINITY;
  ...
  function buildForcedDirectionRectPack(remSource, rect, K, cfg, minScrapW, minScrapH, isCancelled, forcedSwap){
    ...
    const localLimit = Number.isFinite(Number(cfg && cfg.rowSeedLimit)) ? Math.max(4, Math.round(Number(cfg.rowSeedLimit))) : Number.POSITIVE_INFINITY;
  ...
  function buildRectPackVariant(remSource, rect, K, cfg, minScrapW, minScrapH, isCancelled, forcedSwap){
    if(!rect || rect.w <= 40 || rect.h <= 40 || !remSource || !remSource.length) return null;
    let best = null;
    const localLimit = Number.isFinite(Number(cfg && cfg.rowSeedLimit)) ? Math.max(4, Math.round(Number(cfg.rowSeedLimit))) : Number.POSITIVE_INFINITY;
    for(const swap of (typeof forcedSwap === 'boolean' ? [forcedSwap] : [false, true])){
      if(isCancelled && isCancelled()) break;
  ...
  function fillResidualRects(rows, rem, BW, BH, W, H, K, swap, cfg, minScrapW, minScrapH, isCancelled, forcedSwap){
    ...
    for(const rect of baseRects){
      ...
      const variant = buildRectPackVariant(remNow, rect, K, cfg, minScrapW, minScrapH, isCancelled, forcedSwap);
  ...
  function buildSheetVariant(remSource, W, H, K, swap, seed, cfg, minScrapW, minScrapH, isCancelled, isNestedRectPack){
    ...
    if(isNestedRectPack){
      ...
      if(!(extras.placements && extras.placements.length)){
        const residual = fillResidualRects(rows, remNow, BW, BH, W, H, K, swap, cfg, minScrapW, minScrapH, isCancelled, !swap);
        remNow = residual.rem;
        extras = {
          rem: remNow,
  ...
  function chooseSingleCandidate(it, W, H, mode){
    const candidates = [];
    if(it && it.w <= W && it.h <= H) candidates.push({ w: it.w, h: it.h, rotated: false });
    if(it && it.rotationAllowed && it.h <= W && it.w <= H && it.w !== it.h) candidates.push({ w: it.h, h: it.w, rotated: true });
    if(!candidates.length) return null;
    const m = normalizeMode(mode);
    candidates.sort((a,b)=>{
      const score = (c)=> (c.w * c.h) * 1000 + (m === 'along' ? c.w : (m === 'across' ? c.h : Math.max(c.w, c.h))) * 10;
      return score(b) - score(a);
    });
    return candidates[0];
  }

  function buildFallbackSheet(remMaster, W, H, mode){
    if(!Array.isArray(remMaster) || !remMaster.length) return null;
    let bestIndex = -1;
    let bestCand = null;
    let bestScore = -Infinity;
    for(let i=0; i<remMaster.length; i++){
      const it = remMaster[i];
      const cand = chooseSingleCandidate(it, W, H, mode);
      if(!cand) continue;
    ...
    let item = null;
    if(bestIndex >= 0){
      item = remMaster.splice(bestIndex, 1)[0];
    } else {
      item = remMaster.shift();
      if(!item) return null;
      bestCand = { w: Math.min(W, Math.max(1, Number(item.w) || 1)), h: Math.min(H, Math.max(1, Number(item.h) || 1)), rotated: false, oversize: true };
    }
    return {
      boardW: W,
      boardH: H,
      placements: [{
        ...
        x: 0, y: 0,
        w: bestCand.w, h: bestCand.h,
        rotated: !!bestCand.rotated,
        srcW: item.w, srcH: item.h,
        rotationAllowed: !!item.rotationAllowed,
        edgeW1: bestCand.rotated ? item.edgeH1 : item.edgeW1,
        edgeW2: bestCand.rotated ? item.edgeH2 : item.edgeW2,
        edgeH1: bestCand.rotated ? item.edgeW1 : item.edgeH1,
        edgeH2: bestCand.rotated ? item.edgeW2 : item.edgeH2,
        oversize: !!bestCand.oversize,
      }],
      _usedArea: Math.max(0, bestCand.w) * Math.max(0, bestCand.h),
      _freeRects: [],
    };
  }

  function packOptimaCore(itemsIn, boardW, boardH, kerf, options){
    ...
    const mode = normalizeMode(options && (options.mode || options.direction));
    ...
    while(remMaster.length){
      ...
      let best = null;
      const attempts = [];
      for(const swap of getAllowedSwaps(mode)){
        const BW = swap ? H : W;
        const BH = swap ? W : H;
      ...
      if(best && best.sheet && Array.isArray(best.sheet.placements) && best.sheet.placements.length){
        sheets.push(best.sheet);
        remMaster = best.rem;
        continue;
      }
      const fallback = buildFallbackSheet(remMaster, W, H, mode);
      if(!fallback) break;
      sheets.push(fallback);
    }

    return sheets.filter(sh => sh && Array.isArray(sh.placements) && sh.placements.length);
  }

  function packByMode(itemsIn, boardW, boardH, kerf, options){
    const opts = Object.assign({}, options || {});
    opts.mode = normalizeMode(opts.mode || opts.direction);
    return packOptimaCore(itemsIn, boardW, boardH, kerf, opts);
  }
