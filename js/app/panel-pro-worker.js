...
const SOLVER_VER = '20260313_no_time_limits_v1';
try{
...
(function(){
  ...
  function packPanelPro(items, W, H, K, opts){
    ...
    for(const arr of base){
      if(_cancelled || iters >= maxAttempts) break;
      progress.phase = 'main';
      progress.step = 'running';
    ...
    function repackTailWithBorrow(prefixSheets, tailSheets, borrowIds, cutPref){
      ...
      const newTail = packOne(uniq, cutPref);
    ...
    function doStripLastSheetPolish(currentBest){
      ...
      for(const arr of variants){
        if(_cancelled || tailTries >= endgameAttempts) break;
        progress.phase = 'tail';
        progress.step = 'running';
      ...
      while(!_cancelled && tailTries < endgameAttempts){
        progress.phase = 'tail';
        progress.step = 'running';
        progress.currentTailAttempt = Math.min(endgameAttempts, tailTries + 1);
        reportProgress(true);
    ...
    function doOptimaxStripFill(currentBest){
      ...
      const newTail = packOne(remItems, (opts && (opts.cutPref||opts.direction)) || 'along');
    ...
    for(const pref of prefList){
      if(_cancelled) break;
      const sheets = packOne(items, pref);
      const sc = scoreSheets(sheets);
      const res = { sheets, sc };
