...
function getSetParamsFromUI(presetId){
  ...
  const room = state.roomType || (typeof uiState !== 'undefined' ? uiState.roomType : null);
  if(!room) return null;

  // settings may be missing on first run; fall back to defaults
...
function createOrUpdateSetFromWizard(){
  try{
    ...
    // Ensure containers exist early (set wizard depends on room settings)
