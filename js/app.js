...
function registerCoreActions(){
  ...
  // Actions registry handlers are defined in js/app/actions-register.js
  // We pass in the local references that handlers need.
  try{
    if(window.FC && typeof window.FC.actionsRegister === 'function'){
      window.FC.actionsRegister({
        FC,
        uiState,
        STORAGE_KEYS,
        projectData,
    ...
    try{ console.warn('actionsRegister failed', e); }catch(_){}
  };
}
