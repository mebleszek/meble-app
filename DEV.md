...
- `js/app/rozrys/rozrys.js` — odchudzony o lokalne summary/wrappery selection i o blok stanu opcji; nadal trzyma wejście do zakładki, scope, `aggregatePartsForProject`, `generate()` i start renderu panelu.
- `js/testing/rozrys/tests.js` — dodane antyregresje dla obu nowych modułów: (1) selection bridge nadal buduje payload `getScopeSummary/getRoomsSummary` 1:1 dla `rozrysScope` i deleguje wrappery do `selectionUi`, (2) options state nadal zapisuje prefy, przelicza pola po zmianie jednostki i deleguje `getBaseState` 1:1 do `rozrysState`.
- `index.html` + `dev_tests.html` + `tools/app-dev-smoke.js` + `tools/rozrys-dev-smoke.js` — dopięte ładowanie `rozrys-selection-bridge.js` i `rozrys-options-state.js` przed `rozrys.js`, z podbitym cache-bustingiem.
- Instrukcja antyregresyjna: przy kolejnych splitach `ROZRYS` najpierw wynosić całe, powtarzalne wrappery/controller glue (`selection`, `options state`) zamiast wycinać przypadkowe pojedyncze funkcje. Jeśli nowy bridge skleja summary lub zapis stanu, test ma sprawdzać nie tylko pojedynczy helper, ale cały złożony payload i skutki uboczne (`savePanelPrefs`, `setOptionState`, refresh wrapperów).

## 2026-04-19 — ROZRYS output bridge split (render/cache/accordion controller)
- `js/app/rozrys/rozrys-output-bridge.js` — wydzielony bridge outputu ROZRYS: delegacje do `rozrysRender` i `rozrysAccordion` dla `tryAutoRenderFromCache`, `buildEntriesForScope`, `splitMaterialAccordionTitle`, `createMaterialAccordionSection`, `renderMaterialAccordionPlans`, `renderOutput`, `renderLoadingInto`. Dodatkowo `createController(ctx)` skleja payloady z `render()` i pilnuje kontraktu callbacków `renderOutput` / `tryAutoRenderFromCache` bez trzymania tego bloku w `rozrys.js`.
- `js/app/rozrys/rozrys.js` — odchudzony o blok output/render/cache; nadal trzyma wejście do zakładki, scope, pickery, start panelu i `generate()`. Wysokiego ryzyka ścieżki (`aggregatePartsForProject`, `resolveInitialSelectedRooms`, launchery, `generate`) nie były zmieniane funkcjonalnie.
- `js/testing/rozrys/tests.js` — dodane antyregresje dla output bridge: (1) delegacja render/cache 1:1 do `rozrysRender`, (2) delegacja accordionu 1:1 do `rozrysAccordion`, (3) `createController(ctx)` nadal skleja payloady auto-renderu i accordionu z callbackami refresh/render. Test helpera UI dla fake DOM uruchamia klik także przez `__listeners.click[0]`, żeby smoke nie raportował fałszywego błędu przy działającym `info-trigger`.
- `index.html` + `dev_tests.html` + `tools/app-dev-smoke.js` + `tools/rozrys-dev-smoke.js` — dopięte ładowanie `rozrys-output-bridge.js` przed `rozrys.js` i podbity cache-busting.
- Instrukcja antyregresyjna: przy dalszym cięciu `ROZRYS` najpierw wynosić bloki, które już dziś są tylko adapterami do innych modułów (`render`, `accordion`, `lists`, `print`). Jeśli controller w nowym module skleja callbacki zwrotne (`renderOutput`, `tryAutoRenderFromCache`), test ma sprawdzać nie tylko pojedynczą delegację, ale też cały złożony payload `createController(ctx)`.
