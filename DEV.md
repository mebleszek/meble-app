...
- 2026-04-12: ETAP 1 zabezpieczenia statusów projektu — `js/testing/wycena/tests.js` dostał osobny pakiet regresji `Wycena ↔ Statusy projektu`: akceptacja oferty wstępnej i końcowej, cofnięcia `zaakceptowany → wycena` oraz `pomiar → wstepna_wycena`, pełna ścieżka `wstepna_wycena → pomiar → wycena → zaakceptowany`, usuwanie / przełączanie aktywnej oferty oraz test braku zależności od fallbacku do pierwszego pokoju. Do testów wystawiono tylko debug-helpery statusowe w `js/tabs/wycena.js`, a `dev_tests.html` i `tools/app-dev-smoke.js` ładują teraz także ten plik, żeby smoke testy sprawdzały realną logikę tabu Wycena zamiast same store'y.
