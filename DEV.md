## 2026-06-18 — WYCENA: odporność startu po zależnościach v1

- Dodano miękkie ponawianie ładowania podzielonych modułów WYCENY, gdy przeglądarka po wznowieniu starej karty albo chwilowym błędzie sieci uruchomi moduł zależny przed jego wymaganymi częściami.
- Moduły `wycena-core-catalog/source/material-plan/offer/lines/labor` nie rzucają już czerwonego błędu przy samym starcie z powodu chwilowo brakującego modułu zależnego; zamiast tego próbują załadować własny skrypt ponownie po krótkim opóźnieniu.
- `boot.js` dostał odzyskiwanie niezaładowanych skryptów `js/*`: przy błędzie zasobu próbuje pobrać skrypt jeszcze raz z parametrem recovery.
- Nie zmieniano matematyki WYCENY, CZYNNOŚCI, projektu technicznego, transportu, wnoszenia, ORS, PCV ani UI.

Cache-busting: `20260618_wycena_boot_dependency_retry_v1`.

## 2026-06-16 — CZYNNOŚCI: projekt techniczny w Innych czynnościach v1

- W zakładce CZYNNOŚCI pozycje `Projekt techniczny — formatki` i `Projekt techniczny — nietypowa szafka` są teraz doliczane do akordeonu `Inne czynności`.
- Źródłem pozostaje ten sam kolektor co w WYCENIE: `collectProjectPreparationLines`, więc nie ma osobnego liczenia projektu ani duplikatu matematyki.
- Dla bieżącego pomieszczenia CZYNNOŚCI pokazują normoczas projektu z jego szafek; bez bieżącego pomieszczenia fallbackiem jest wybór pomieszczeń z draftu oferty.
- Transport kilometrowy nadal nie trafia do czasu `Innych czynności`; czas dojazdu zostaje widoczny jako czas.
- Nie ruszano matematyki WYCENY, transportu, wnoszenia, ORS, PCV, PDF, szuflad, cargo, klap ani skręcania korpusów.
...
- `wycena-diagnostics` ma build `20260618_wycena_boot_dependency_retry_v1`, liczniki cache i rozmiary `snapshot`, `calculationRegister`, `labor`,
