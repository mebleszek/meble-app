...
- 2026-03-22: ROZRYS action row restyled from `site_rozrys_action_row_fix.zip` — kept single-row order `Opcje -> Dodaj płytę -> Generuj ponownie`, restored shorter button height closer to accepted `Generuj ponownie`, changed row buttons to light cards with black labels and subtle neutral/green/blue glow, updated cache busting for `css/style.css` and `js/app/rozrys.js`.
