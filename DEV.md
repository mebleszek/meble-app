## 2026-04-19 — ROZRYS accordion bridge split (accordion/result section wrappers)
- `js/app/rozrys/rozrys-accordion-bridge.js` — wydzielony lekki bridge dla wrapperów akordeonów wyników: `splitMaterialAccordionTitle`, `createMaterialAccordionSection`, `renderMaterialAccordionPlans`. To jest tylko warstwa sklejenia payloadu do `rozrys-accordion`, bez ruszania pickerów, launcherów, scope ani `generate()`.
- `js/app/rozrys/rozrys.js` — zamiast trzymać lokalne wrappery akordeonów buduje `accordionBridge` po utworzeniu `out` i czyta z niego trzy helpery. Bootstrap launcherów (`updateRoomsPickerButton`, `updateMaterialPickerButton`, `syncHiddenSelections`, klik rooms/material`) zostaje w tym samym miejscu i nie jest dotykany.
- `js/testing/rozrys/tests.js` — dodane antyregresje: (1) bridge przekazuje 1:1 payload do `rozrysAccordion`, w tym `scheduleSheetCanvasRefresh`, `out`, `tryAutoRenderFromCache`, `openMaterialGrainExceptions`, `renderOutput`; (2) `index.html` i `dev_tests.html` ładują `rozrys-accordion-bridge.js` przed `rozrys.js`; (3) bootstrap launcherów dalej jest obecny po splicie akordeonów.
- `index.html` + `dev_tests.html` + `tools/*smoke.js` — dopięte ładowanie/asset source nowego bridge przed `rozrys.js`.
- Instrukcja antyregresyjna: przy dalszych splitach renderu wyników nie przenosić razem wrapperów akordeonów i logiki bootstrapu/selection. Najpierw sprawdzić kto konsumuje `splitMaterialAccordionTitle`, `createMaterialAccordionSection`, `renderMaterialAccordionPlans` (scope, pickery, runner, render), a dopiero potem wynosić ten blok.
