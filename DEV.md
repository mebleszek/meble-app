...
- Anti-regression: do not create a second backup implementation later. Normal program, tests, imports, restore, migrations and future cloud/data tools must use `FC.dataBackupStore` / `FC.dataBackupSnapshot`. Tests may cleanup only marked test data, never unmarked user data. Before import/restore, always create a backup of the current state.
