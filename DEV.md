...
## 2026-04-23 — registry / roomless WYCENA recovery after broken asset wiring
- `index.html` + `dev_tests.html` — po ręcznych poprawkach assetów nie wolno zostawiać placeholderów w `src` skryptów. W tym regresie surowy placeholder zastąpił realne ścieżki (`room-registry-definitions`, `app-room-render`, `wywiad`, `czynnosci`, `wycena`, testy), co rozpięło runtime i dało objawy pozornie niezwiązane z logiką: puste pokoje, brak działania przycisków, zły routing `WYCENA`. Po każdej ręcznej zmianie assetów sprawdzać grepem, czy w `index.html` / `dev_tests.html` nie zostały obce placeholdery i czy `tools/check-index-load-groups.js` przechodzi.
- `js/app/shared/room-registry-foundation.js` — `getCurrentInvestor()` nie może polegać wyłącznie na `investors.getCurrentId()`. Ma umieć odzyskać inwestora także z `investorPersistence`, `uiState.currentInvestorId` i `investorUI.state`, bo inaczej modale pokoi przestają działać mimo że detal inwestora jest otwarty.
- `js/app/shared/room-registry-definitions.js` — przy łączeniu `project meta` i `investor.rooms` nie zgubić lepszych nazw z meta. Nie robić prostego `Object.assign(projectRoom, investorRoom)`, bo techniczny `room_*` z inwestora nadpisze czytelną etykietę. Mergować oba źródła przez wspólne wybieranie lepszego tekstu.
- `js/app/ui/tab-navigation.js` — przejście do roomless `WYCENA` ma umieć wywnioskować kontekst inwestora z persistence/UI, nawet jeśli `uiState.currentInvestorId` chwilowo jest puste. Inaczej `setActiveTab('wycena')` wraca do `roomsView` zamiast do właściwego działu.
