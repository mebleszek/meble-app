...
- Aktualna paczka robocza po tym etapie: `site_wycena_click_no_output_fix_v1.zip`.
- Baza startowa: `site_hardware_hinge_dictionary_cleanup_v1.zip`.
- Naprawiono regresję, w której kliknięcie zielonego `Wyceń` mogło dawać reakcję przycisku, ale nie pokazywać wygenerowanego podglądu wyceny.
- `js/app/wycena/wycena-tab-shell.js` po wygenerowaniu snapshotu zapisuje jawnie `lastQuote`, `previewSnapshotId` i przewinięcie do podglądu, a po synchronizacji statusu projektu odświeża baseline statusu. Dzięki temu świeżo utworzony snapshot nie jest czyszczony przez `reconcileStatusPreviewState`.
- Walidacyjne błędy tworzenia wyceny nadal korzystają z aplikacyjnego modala, ale dodatkowo są wpisywane jako widoczny błąd w sekcji WYCENY, żeby kliknięcie nie wyglądało jak brak reakcji.
- `js/app/wycena/wycena-tab-render-bridge.js` nie ukrywa już po cichu błędów renderu podglądu/historii/shella: loguje błąd i pokazuje kartę `Błąd podglądu WYCENY`.
- Poprawiono test `js/testing/cabinet/tests.js`, który po zmianie nazw zawiasów nadal szukał starego `Zawias BLUM`; teraz sprawdza zawias po `hardwareRequirement.kind === 'hinge'` albo po nazwie `Zawias...`.
- Dodano regresyjny test kliknięcia `tools/wycena-click-regression-smoke.js`, który renderuje WYCENĘ, klika `Wyceń` i sprawdza, że powstaje snapshot, podgląd `Podsumowanie/Razem` oraz `Historia wycen`.
- Rozszerzono `tools/app-dev-smoke.js` o kontrakt pilnujący zachowania świeżo wygenerowanego podglądu po synchronizacji statusu i braku cichego połykania błędów renderu.
- Zaktualizowano cache-busting `wycena-tab-render-bridge.js`, `wycena-tab-shell.js` oraz `js/testing/cabinet/tests.js` w `dev_tests.html`.
- Nie ruszano panelu `Kategorie / rodzaje okuć`, backupów, storage, import/export Excel, PRO100, usług stolarskich, ROZRYS, RYSUNKU ani resolvera konkretnych pozycji okuć do WYCENY.
