## 2026-04-19 — pakiet ratunkowy listy inwestorów (recovery z projektów/snapshotów)
- `js/app/investor/investor-store-recovery.js` — nowy moduł odzysku listy inwestorów. Czyta główny store, backup `fc_investors_backup_v1` i — gdy lista wygląda na uciętą — odbudowuje brakujące rekordy z `fc_projects_v1` oraz `fc_quote_snapshots_v1`, wzbogacając odzyskane wpisy o nazwę/telefon/adres oraz pokoje/scope. Zapisuje naprawioną listę z powrotem do `fc_investors_v1` bez kasowania już widocznych nowych inwestorów.
- `js/app/investor/investors-store.js` — `readAll()` nie ufa już ślepo tylko `fc_investors_v1`; najpierw parsuje listę defensywnie, potem przepuszcza ją przez recovery i zapisuje backup przy każdym `writeAll`. Dodane `getRecoveryState()` do debugowania.
- `js/testing/investor/tests.js` — nowe antyregresje dla dwóch scenariuszy: (1) brakujące stare rekordy wracają z `projectStore` + snapshotów obok istniejącego nowego inwestora, (2) całkiem pusta lista inwestorów odbudowuje się z samych snapshotów ofert.
- `index.html` + `dev_tests.html` + `tools/app-dev-smoke.js` — dopięte ładowanie nowego modułu przed `investors-store.js` i podbity cache-busting.
- Instrukcja antyregresyjna: przy danych inwestorów nie zakładać, że `fc_investors_v1` zawsze jest kompletne. Jeśli lista inwestorów i dane projektów/snapshotów mogą się rozjechać, najpierw robić recovery/merge po ID inwestora, a dopiero potem render listy. Nie odtwarzać rekordów z samych luźnych slotów projektu, jeśli nie ma innych śladów danego inwestora — to łatwo wskrzesza stare śmieci testowe.
