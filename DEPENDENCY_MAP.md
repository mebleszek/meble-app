...
- `js/app.js` — po kolejnych splitach ma ok. 354 linie, średnie ryzyko i nadal jest app shellem/runtime klejem.
- `js/app/bootstrap/app-core-namespace.js` przejął bootstrap `FC`/fallback storage/project i musi ładować się przed `js/app.js`.
- `js/app/bootstrap/app-legacy-bridges.js` przejął globalne delegatory cabinet/material/price i musi ładować się po `js/app.js` w głównym runtime; zachowuje stare globalne nazwy funkcji bez implementacji domenowej.
- Po stage 1, app core split i legacy bridges split nie ma już bezpośredniego `localStorage`/`sessionStorage` w `app.js`, ale shell pozostaje wpływowy.
- Kolejne porządki powinny wyjmować tylko mechaniki boczne o jasnym kontrakcie: runtime validation/state, widoki, routing stanu, inicjalizacja sekcji.
...
- Dodano `js/app/bootstrap/app-core-namespace.js` jako boundary bootstrapu namespace `FC`, fallback storage i fallback projektu.
- `index.html` i `tools/index-load-groups.js` ładują go bezpośrednio przed `js/app.js`.
- `app-dev-smoke` ma kontrakt obecności `FC.appCoreNamespace.createAppCore`.
- Ręcznie sprawdzono runtime load w uproszczonym sandboxie przez warstwy `startup-foundation`, `business-domains`, `ui-shell`, `app-runtime` do `js/app.js`.
- Plik ma duży wpływ pośredni, więc nie dokładać do niego logiki domenowej. Ma pozostać małym bootstrap/fallback boundary.


## 2026-05-02 — App legacy bridges split v1

- Dodano `js/app/bootstrap/app-legacy-bridges.js` jako boundary zgodności dla globalnych delegatorów cabinet/front, cabinet modal/actions, price modal i material/front hardware.
- `index.html`, `dev_tests.html`, `tools/index-load-groups.js` i `tools/app-dev-smoke-lib/file-list.js` znają nowy moduł.
